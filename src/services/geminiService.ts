...
export const generateDishImage = async (
  ...
  if (logoBase64) {
    ...
    refLegend.push(`REFERENCE IMAGE #${logoIndex} = RESTAURANT LOGO (MANDATORY USE). You MUST integrate this exact logo into the final image — failure to include it is unacceptable. MICHELIN NAPKIN BRANDING PROTOCOL: render a crisp, freshly-pressed folded white linen napkin LYING FLAT AND FULLY RESTING ON THE PHYSICAL TABLE SURFACE next to the plate — the napkin MUST be in direct contact with the table, casting a soft natural contact shadow on the table. ABSOLUTELY FORBIDDEN: floating, levitating, hovering, tilted, mid-air, suspended, flying, or angled napkin. The napkin obeys gravity at all times and sits naturally on the same flat surface as the plate, cutlery and glassware. On that napkin, place THIS EXACT logo from REFERENCE IMAGE #${logoIndex}, reproduced as a subtle single-tone embroidery or fine tone-on-tone print in muted charcoal-grey or warm ivory thread. Preserve the logo's exact shape, proportions, letters, symbols and orientation 1:1 — do NOT redraw, restyle, mirror, recolor with extra colors, translate, or invent any text. Size: roughly 25–35% of the napkin's visible area, centered on one folded panel. The napkin must have soft natural folds, visible fine linen weave, and a gentle realistic contact shadow where it meets the table. Refined, understated, fine-dining presentation. Never overlay the logo on the food, plate, cutlery, or background. The entire scene (plate, napkin, cutlery, glassware, props) must be naturally placed on the table — nothing floats, nothing defies gravity.`);
  }
  if (locationBase64) {
  ...
  const logoCritical = logoBase64
    ? `\n  [CRITICAL — LOGO ON NAPKIN]: The final image MUST contain a folded white linen napkin LYING FLAT ON THE TABLE beside the dish (fully resting on the table surface, with a natural contact shadow — NEVER floating, levitating, tilted or in mid-air), with the exact logo from REFERENCE IMAGE #${logoIndex} subtly embroidered onto it in a single muted tone. Gravity must be respected for every object in the scene. This is non-negotiable.`
    : "";
