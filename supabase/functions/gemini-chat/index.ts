...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: creditResult, error: creditError } = await supabase.rpc('deduct_credit', { p_amount: 1, p_reason: 'Chat with AI Concierge' });
    ...
    if (message.length > MAX_MESSAGE_LENGTH) {
      return new Response(JSON.stringify({ error: `message must be under ${MAX_MESSAGE_LENGTH} characters` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (history !== undefined && history !== null && !Array.isArray(history)) {
      return new Response(JSON.stringify({ error: "history must be an array" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (Array.isArray(history) && history.length > MAX_HISTORY_LENGTH) {
      return new Response(JSON.stringify({ error: `history must have at most ${MAX_HISTORY_LENGTH} entries` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (imageBase64 !== undefined && imageBase64 !== null) {
      if (typeof imageBase64 !== "string") {
        return new Response(JSON.stringify({ error: "imageBase64 must be a string" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
    ...
    const messages: any[] = [
      { role: "system", content: "You are Maya, the expert AI Concierge for Instant Menu Pictures. Analyze any images with Michelin standards. Keep responses concise but intellectually deep." },
    ];
    ...
    for (const h of (Array.isArray(history) ? history : [])) {
      if (h && typeof h.role === "string" && typeof h.text === "string") {
        messages.push({
          ...
          content: String(h.text).slice(0, MAX_HISTORY_TEXT_LENGTH),
        });
      }
    }
    ...
    const resp = await fetch("https://api.moonshot.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: imageBase64 ? "kimi-latest" : "kimi-k2-0905-preview",
        temperature: 0.6,
        messages,
      }),
    });
    ...
    return new Response(JSON.stringify({ error: "Chat processing failed" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
